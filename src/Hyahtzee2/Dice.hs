...
import qualified Hyahtzee2.Util as Util (generateRandomValues, modifyNth)
...
throwDice randomGen =
  let (values', randomGen') = Util.generateRandomValues 5 randomGen
   in (Dice $ map (,False) values', randomGen')
...
rethrow (Dice dice) randomGen =
  let amountToThrow = length (filter (not . snd) dice)
   ...
   in (Dice $ keptDice ++ newDice, randomGen')
